...
/**
 * Salon krokisi duzenleyici.
 *
 * Konum yuzde olarak tutulur (0-100), piksel degil: tuval ekran genisligine
 * gore esner. Piksel saklansaydi tablette dizilen yerlesim masaustunde
 * dagilirdi.
 *
 * Her birakis kendi basina kaydedilir (TABLE_POSITION_SET komutu). Toplu
 * "Kaydet" dugmesi yok: 30 masali salonda tek tek surukleyip en sonda
 * kaydetmeyi unutmak tum emegi cope atardi.
 */
...
export function TableFloorPlanEditor({
  ...
  const [positions, setPositions] = useState<Record<string, Point>>(() => {
    ...
    // Varsayilan dizilim bolge icinde hesaplanir: her bolge kendi krokisine
    // sahip, indeksler bolgeler arasi devam etseydi ikinci bolge tuvalin
    // disina dizilirdi.
  ...
  /** Imlec konumunu tuval yuzdesine cevirir. Kenarlardan tasma engellenir. */
  function pointerToPercent(clientX: number, clientY: number): Point | null {
    const canvas = canvasRef.current;
    if (!canvas) return null;
    const rect = canvas.getBoundingClientRect();
    return {
      x: Math.min(96, Math.max(0, ((clientX - rect.left) / rect.width) * 100)),
      y: Math.min(94, Math.max(0, ((clientY - rect.top) / rect.height) * 100)),
    };
  }
  ...
  function handlePointerMove(event: React.PointerEvent<HTMLDivElement>) {
    ...
    if (!tableId) return;
    const next = pointerToPercent(event.clientX, event.clientY);
    if (!next) return;
  ...
  async function handlePointerUp(event: React.PointerEvent<HTMLDivElement>) {
    ...
    // Konum birakis olayindan yeniden hesaplanir, state'ten okunmaz: son
    // pointermove ile pointerup ayni karede olursa `positions` kapanisi bir
    // kare geride kalir ve masa gorunenden farkli bir yere kaydedilirdi.
    const finalPosition = pointerToPercent(event.clientX, event.clientY) ?? positions[tableId];
    if (!finalPosition) return;
    setPositions((prev) => ({ ...prev, [tableId]: finalPosition }));
    setDirty((prev) => new Set(prev).add(tableId));
    await persistPosition(tableId, finalPosition);
  }

  async function persistPosition(tableId: string, position: Point) {
  ...
  /** Bu bolgenin masalarini izgaraya dizer ve her birini kaydeder. */
  async function resetLayout() {
    const nextPositions: Record<string, Point> = {};
    zoneTables.forEach((table, index) => {
      nextPositions[table.id] = defaultPosition(index);
    });
    ...
    // Otomatik dizilim de kaydedilir. Eskiden yalnizca ekranda diziliyordu;
    // sayfa yenilenince eski yerlesim geri geldigi icin dugme bozuk sanilirdi.
    for (const table of zoneTables) {
      const position = nextPositions[table.id];
      if (position) {
        await persistPosition(table.id, position);
      }
    }
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Her bölge (kat/alan) kendi bağımsız krokisine sahiptir. Masaları sürükleyip bırakarak yerleşimi oluşturun,
            her bırakışta konum otomatik kaydedilir.
          </p>
        </div>
          ...
          zoneTables.map((table) => {
            ...
            return (
              ...
              >
                ...
                <select
                  value={seatCount}
                  aria-label={`Masa ${table.table_number} kapasitesi`}
