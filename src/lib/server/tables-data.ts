...
export async function getTableMapImpl(deps: QueryDeps) {
  ...
  const reader = unstable_cache(
    async () => {
      ...
      try {
        let result = (await deps.withQueryTimeout(buildQuery(true, true, true))) as { data: unknown[] | null; error: { message: string } | null };
        if (result.error?.message?.toLowerCase().includes("seat_count")) {
          result = (await deps.withQueryTimeout(buildQuery(true, true, false))) as { data: unknown[] | null; error: { message: string } | null };
        }
        if (result.error?.message?.toLowerCase().includes("position_x") || result.error?.message?.toLowerCase().includes("position_y")) {
          result = (await deps.withQueryTimeout(buildQuery(true, false, false))) as { data: unknown[] | null; error: { message: string } | null };
        }
        if (result.error?.message?.toLowerCase().includes("zone_id")) {
          result = (await deps.withQueryTimeout(buildQuery(false, false, false))) as { data: unknown[] | null; error: { message: string } | null };
        }
...
export async function createTableImpl(
  ...
  if (firstInsert.error?.message?.toLowerCase().includes("zone_id")) {
    if (resolvedZone.zoneId) {
      return { ok: false, error: "Bölge ozelligi için veritabani migrasyonunu çalıştırın." };
    }
  ...
  if (!data) {
    return { ok: false, error: "Masa oluşturulamadı." };
  }
...
export async function createTableZoneImpl(name: string, deps: MutationDeps): Promise<CreateZoneResult> {
  const supabase = getSupabaseServerClient();
  if (!supabase) {
    return { ok: false, error: "Demo modda bölge oluşturma pasif." };
  }

  const normalizedName = name.trim().replace(/\s+/g, " ");
  if (normalizedName.length < 2) {
    return { ok: false, error: "Bölge adi en az 2 karakter olmali." };
  }
  ...
  if (error) {
    if (error.message.toLowerCase().includes("table_zones_business_branch_name_unique")) {
      return { ok: false, error: "Bu isimde bir bölge zaten var." };
    }
    return { ok: false, error: error.message };
  }
...
export async function assignTableZoneImpl(
  ...
  const supabase = getSupabaseServerClient();
  if (!supabase) {
    return { ok: false, error: "Demo modda bölge atama pasif." };
  }
...
export async function bulkCreateTablesImpl(
  ...
  if (!Number.isInteger(input.startNumber) || input.startNumber <= 0) {
    return { ok: false, error: "Başlangıç masa no pozitif bir tam sayi olmali." };
  }
...
export async function bulkDeleteTablesImpl(
  ...
  if (input.endNumber < input.startNumber) {
    return { ok: false, error: "Bitis masa no, başlangıç no'dan küçük olamaz." };
  }
  if (input.endNumber - input.startNumber > 1000) {
    return { ok: false, error: "Tek seferde en fazla 1001 masa aralığı silinebilir." };
  }
...
export async function deleteTableZoneImpl(zoneId: string, deps: MutationDeps): Promise<DeleteZoneResult> {
  const supabase = getSupabaseServerClient();
  if (!supabase) {
    return { ok: false, error: "Demo modda bölge silme pasif." };
  }
...
export async function bulkDeleteTableZonesImpl(
  ...
  const supabase = getSupabaseServerClient();
  if (!supabase) {
    return { ok: false, error: "Demo modda toplu bölge silme pasif." };
  }
...
export async function updateTableDetailsImpl(
  input: { tableId: string; tableNumber: number; name: string; seatCount?: number },
  deps: MutationDeps,
) {
  const supabase = getSupabaseServerClient();
  if (!supabase) {
  ...
  if (input.seatCount !== undefined) {
    updatePayload.seat_count = input.seatCount;
  }
  let query = supabase
    .from("tables")
  ...
  if (error?.message?.toLowerCase().includes("seat_count") && input.seatCount !== undefined) {
    delete updatePayload.seat_count;
    let retryQuery = supabase.from("tables").update(updatePayload).eq("id", input.tableId);
    if (!scope.useLegacySchema && scope.businessId) {
      retryQuery = retryQuery.eq("business_id", scope.businessId);
    }
    if (targetBranchId) {
      retryQuery = retryQuery.eq("branch_id", targetBranchId);
    }
...
export async function updateTablePositionImpl(
  ...
  if (error) {
    if (error.message?.toLowerCase().includes("position_x")) {
      return { ok: false, error: "Kroki ozelligi için veritabani migrasyonunu çalıştırın." };
    }
    return { ok: false, error: error.message };
  }
...
export async function updateTableSeatCountImpl(
  ...
  if (error) {
    if (error.message?.toLowerCase().includes("seat_count")) {
      return { ok: false, error: "Kapasite ozelligi için veritabani migrasyonunu çalıştırın." };
    }
    return { ok: false, error: error.message };
  }
...
export async function deleteTableImpl(tableId: string, deps: MutationDeps) {
  const supabase = getSupabaseServerClient();
  if (!supabase) {
    return { ok: false, error: "Demo modda silme işlemi pasif." };
  }
...
export async function moveTableOrderImpl(
  ...
  if ((targetTable.status as TableStatus) !== "empty") {
    return { ok: false, error: "Adisyon sadece boş bir masaya tasinabilir." };
  }
  ...
  if (!claimedTargetRows || claimedTargetRows.length === 0) {
    return { ok: false, error: "Hedef masa az önce baska bir işlem tarafindan dolduruldu." };
  }
