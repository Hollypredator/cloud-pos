...
export default async function StudioLoginPage({
  ...
  if (user?.email && (await getStudioAccessByEmail(user.email)).hasAccess) {
    redirect(next && next.startsWith("/") ? next : "/studio/onboarding");
  }
        ...
        <div className="grid w-full gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <section className="rounded-[2rem] border border-white/10 bg-white p-8 text-slate-900 shadow-[0_25px_70px_rgba(15,23,42,0.28)]">
            ...
            <form action="/auth/login" method="post" className="mt-8 space-y-4">
              <input type="hidden" name="next" value={next ?? "/studio/onboarding"} />
              <div className="space-y-2">
                <label htmlFor="studio-email" className="text-sm font-medium text-slate-700">E-posta</label>
