    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-8 md:px-8">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            {updates.map((update) => (
              <div key={update.id} className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm font-semibold text-slate-900">{update.author_name || "Support"}</p>
        ...
        <form action={addIncidentUpdateAction} className="rounded-2xl bg-white p-6 shadow-sm">
          ...
          <p className="text-sm font-semibold text-slate-900">Update / Postmortem Notu</p>
          ...
          <textarea name="message" rows={8} className="mt-4 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900" placeholder="Update, aksiyon veya postmortem notu ekleyin" />
          <div className="mt-4 flex justify-end">
            <button type="submit" className="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white">Update Kaydet</button>
          </div>
        </form>
      </section>
    </main>
