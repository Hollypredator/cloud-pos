...
function stationLabel(station: KitchenStation) {
  ...
  if (station === "dessert") return "Tatlı";
  return "Mutfak";
}
...
export default async function KitchenPage({
  ...
  if (!featureAccess.enabled) {
    ...
    return (
      <BackofficePage title="Mutfak" description="İstasyon bazlı hazırlama akışı" minimal={isTabletMode}>
        <FeatureLockedState
  ...
  return (
    <>
      <BackofficePage
        title="Mutfak Board"
        description="İstasyon bazlı hazırlama kuyrugu, gecikmeler ve servis çıkışları"
      ...
      >
        ...
        <section className="app-mobile-hide grid gap-4 xl:grid-cols-4">
          ...
          <SummaryCard label="Hazırlanıyor" value={String(preparingCount)} hint="İstasyonda islenen sipariş" tone="neutral" />
          <SummaryCard label="Servise Hazır" value={String(servedCount)} hint="Tamamlandı ama kasaya devredilmedi" tone="success" />
        ...
        <section className="app-mobile-only space-y-3">
          ...
          <article className="mobile-task-card">
            <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-slate-500">Aktif İstasyon</p>
        ...
        <ContentCard title="İstasyon Board" className="app-mobile-hide">
          {orders.length === 0 ? (
              ...
              {stationBoards.map((board) => (
                <section key={board.key} className="rounded-[24px] border border-slate-200 bg-[#f7f8fa] p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">İstasyon</p>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="mt-4 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
                              <div>
                                <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">İstasyon Tutarı</p>
                            ...
                            {stationStatus === "served" ? (
                              <div className="mt-3 flex flex-col items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                  ...
                                  <p className="mt-1 text-emerald-700">Yanlış baskı veya son dakika düzeltmesi için mutfaktan geri alınabilir.</p>
                                </div>
