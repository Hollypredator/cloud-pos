-- Add order item cost snapshots for product-level profitability.
...
begin
  if not exists (
    ...
    alter table public.order_items
      add column unit_cost_snapshot numeric(12,4) not null default 0;
  end if;
end $$;
...
begin
  if not exists (
    ...
    where table_schema = 'public'
      and table_name = 'order_items'
      and column_name = 'line_cost_snapshot'
  ) then
    alter table public.order_items
      add column line_cost_snapshot numeric(14,4) not null default 0;
  end if;
end $$;
...
with product_unit_cost as (
  select
    ...
    greatest(0, coalesce(p.cost, 0))
      + coalesce(sum(greatest(0, coalesce(pi.quantity, 0)) * greatest(0, coalesce(i.cost, 0))), 0) as unit_cost
...
set
  unit_cost_snapshot = round(greatest(0, coalesce(puc.unit_cost, 0))::numeric, 4),
  line_cost_snapshot = round((greatest(0, coalesce(puc.unit_cost, 0)) * greatest(0, coalesce(oi.quantity, 0)))::numeric, 4)
from product_unit_cost puc
where oi.product_id = puc.product_id;
