...
- The current mobile docs require one primary action per screen; several operational surfaces still expose multi-action card stacks and desktop-style seçondary controls.
...
- Mobile `Sipariş A?` must be table-first on phone. Opening the composer before table selection or using the tablet three-pane layout makes the flow look like the wrong screen.
...
- The existing renderer is CMS-oriented and restaurant-owner focused: pricing cards, restaurant copy, rounded SaaS card grids, large gradients, and mega-menü content.
...
- Strong trust signals available without inventing proof: real operations panel screenshots, mobile PWA screenshots, self-service / QR flows, cafe-restaurant modules, multi-branch model, role-based modules, kitchen/cashier/order/inventory/reporting coverage, seçurity headers, and route guards.
...
- Broad replacements can corrupt code identifiers when Turkish-like substrings appear in English code words such as `seçtion`, `loading`, `seçure`, or `radius`; typecheck is required after any mechanical Turkish text pass.
...
- `src/components/product-landing-page.tsx` references Turkish-character filenames: `/landing-assets/mobil-pos-siparis.png` and `/landing-assets/mobil-masa-akisi.png`.
- `src/components/demo-page-renderer.tsx` also references `/landing-assets/mobil-pos-siparis.png`.
...
- QR menü visibility must be enforced server-side in `/{slug}/qr/{identifier}` so public users do not receive product/menü data when QR menü is disabled.
...
- Existing local QR demo route `/qr/table-1` redirects to `/default/qr/table-1` and renders a large seeded menü, which is suitable for visual QA.
...
## 2026-06-13 Landing Copy and Speed Pass
- Visible landing copy had a real Turkish spelling issue: `adısyon` should be `adisyon`.
- `/` was awaiting `getSitePageContent("home")` even though the result was unused after switching the public homepage to `ProductLandingPage`; removing it reduces avoidable server work.
- The landing used plain `<img>` tags for local screenshots. Switching to `next/image` gives responsive image generation, priority control for the hero image, and lazy loading for below-fold screenshots.
- `public/landing-assets/mobil-masa-akisi.png` is 780x11110 and 607KB. The landing card only shows a cropped preview, so a 780x1688 preview asset at about 90KB is a better fit.
- Local perf guard with no auth cookie shows `/` well under the page budget; `api.metrics_ops` fails with 401 because authenticated perf baseline needs `PERF_AUTH_COOKIE`.
